...
| Cell-level mutations | `setValue`, `clearValue`, `clearValues`, `setStyle`, `setFormat`, `setBorder`, `applyStyle`, `applyCustomFormat` | Supported | Includes single-cell and bounded rectangular range clear/style/format/border roundtrip paths |
| Table structure and layout mutations | `appendRow`, `insertRow`, `appendColumn`, `insertColumn`, `deleteRow`, `deleteColumn`, `setHeaderRowCount`, `setHeaderColumnCount`, `setRowHeight`, `setColumnWidth` | Supported / guarded | Bounds validation and deterministic updates; `insertColumn(_:at:)` is currently a deterministic unsupported-write guard until native column shifting lands |
...
| Safety guards | Grouped/pivot-linked mutation protection (`groupedTableMutationUnsupported`, `pivotLinkedTableMutationUnsupported`) + formula write safety validation | Supported | Unsafe structural writes are blocked with clear errors; grouped delete diagnostics include operation index context; pivot-linked guard errors include target-scoped linked object identifiers for triage; unsafe formula references (sheet-qualified/self-referential single-cell or range references) are rejected deterministically |
