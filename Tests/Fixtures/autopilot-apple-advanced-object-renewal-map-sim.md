...
|---|---|---|---|---|
| chart-object-discovery | available | `chart` | safe-read-only | `swiftnumbers inspect` chart inventory diagnostics; native chart writes remain unsupported |
...
| shape-line-text-object-discovery | available | `shape`, `line`, `text item` | safe-read-only | `swiftnumbers inspect` shape/line/text inventory diagnostics; native object writes remain unsupported |
